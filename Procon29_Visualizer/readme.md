# Procon29 Visualizer (ver. 6.0)

## What's new

前のバージョンとボットの互換性がないため、バージョンが1上がっています。  
これ以降のバージョンでは、なるたけ互換性を保つように、仕様が変更していきます。  
今後ともVisualizerをよろしくお願いします。  

### ボット対応によるVisualizer側の仕様変更

てげてげボットがくそみたいなリクエストばかりするから、  
Visualizerのエラー判定が鍛えられた

### ボットの仕様変更

Version 5.0との互換性はないので、ボット製作者は注意すること。  
後でWikiに書くから、ちゃんと見るんだぞ

### オレンジチームもボットが選択できるようになった

オレンジチーム側のエージェントから、  
「ライムチームばかりずっる～い！」という意見があったため、実装した。  
ここだけの話、エージェントはJKなのだろうか？  
……わかった、今度、弟に聞いてみるよ。

### ボットで相手のタイルを取り除いてないのにエージェントが移動できるバグを修正

ボットだけがこういう不正行為をして、  
真面目にやっている人がバカみたいになってしまうのを防ぐために、  
僕はここにいるのさ。
