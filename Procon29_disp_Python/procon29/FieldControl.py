...
class Field:
    ...
    def __init__(self, qrdata, field, logfile):
        ...
        for i in range(self.y+2):
            for j in range(self.x+2):
                self.field_out.append(j*100+i)
                if i != 0 and j != 0 and i != self.y+1 and j != self.x+1:
                    self.field_size.append(j*100+i)
        ...
        for i in range(self.y):
            self.panel.append([wx.Button(field, (j+1)*100+(i+1), str(self.point[i][j]), size=(50,50), pos=(50*j,50*i))\
    ...
    def Coloring(self, agent_data, agent, out_get=False, out_now=False):
        ...
        if out_get == False:
            ...
            for i in range(len(agent_data.GetPosition)):
                self.panel[agent_data.GetPosition[i]%100-1][int(agent_data.GetPosition[i]/100)-1]\
                .SetBackgroundColour(agent_data.GetColor)
                self.log.LogWrite('Coloring of get ({},{}):{}\n'\
                                .format(int(agent_data.GetPosition[i]/100), agent_data.GetPosition[i]%100, agent_data.GetColor), logtype=procon29.SYSTEM_LOG)
        if out_now == False:
            self.NowHide(agent_data, agent)
        else:
            for i in range(2):
                self.panel[int(agent[i].now%100)-1][int(agent[i].now/100)-1]\
                .SetBackgroundColour(agent_data.Color)
                self.log.LogWrite('Coloring of now ({},{}):{}\n'\
                                .format(int(agent[i].now/100), agent[i].now%100, agent_data.Color), logtype=procon29.SYSTEM_LOG)
    ...
    def FillColoring(self, agent_data, out=False):
        ...
        if out == False:
            ...
            for i in range(len(agent_data.GetField)):
                self.panel[int(agent_data.GetField[i]%100)-1][int(agent_data.GetField[i]/100)-1]\
                .SetBackgroundColour(agent_data.FillColor)
                self.log.LogWrite('Coloring of Fill ({},{}):{}\n'\
                                .format(int(agent_data.GetField[i]/100), agent_data.GetField[i]%100, '#FFFFFF'), logtype=procon29.SYSTEM_LOG)
            ...
            for i in range(2):
                for j in range(len(agent[i].movable)):
                    if agent[i].movable[j] not in agent_data.GetPosition and agent[i].movable[j] != agent[i].next:
                        self.log.LogWrite('Coloring of movable ({},{}):{}\n'\
                                        .format(int(agent[i].movable[j]/100), agent[i].movable[j]%100, agent_data.MovableColor), logtype=procon29.SYSTEM_LOG)
                        self.panel[agent[i].movable[j]%100-1][int(agent[i].movable[j]/100)-1]\
                ...
                for j in range(len(agent[i].removable)):
                    if agent[i].removable[j] not in agent_data.GetPosition:
                        self.panel[int(agent[i].removable[j]%100)-1][int(agent[i].removable[j]/100)-1]\
                        .SetBackgroundColour(agent_data.RemovableColor)
                        self.log.LogWrite('Coloring of removable ({},{}):{}\n'\
                                        .format(int(agent[i].removable[j]/100), agent[i].removable[j]%100, agent_data.RemovableColor), logtype=procon29.SYSTEM_LOG)
        else:
            for j in range(len(agent.movable)):
                if agent.movable[j] not in agent_data.GetPosition and agent.movable[j] != agent.next:
                    self.log.LogWrite('Coloring of movable ({},{}):{}\n'\
                                    .format(int(agent.movable[j]/100), agent.movable[j]%100, agent_data.MovableColor), logtype=procon29.SYSTEM_LOG)
                    self.panel[agent.movable[j]%100-1][int(agent.movable[j]/100)-1]\
                    .SetBackgroundColour(agent_data.MovableColor)
            for j in range(len(agent.removable)):
                if agent.removable[j] not in agent_data.GetPosition:
                    self.panel[int(agent.removable[j]%100)-1][int(agent.removable[j]/100)-1]\
                    .SetBackgroundColour(agent_data.RemovableColor)
                    self.log.LogWrite('Coloring of removable ({},{}):{}\n'\
                                    .format(int(agent.removable[j]/100), agent.removable[j]%100, agent_data.RemovableColor), logtype=procon29.SYSTEM_LOG)
        ...
        for i in range(2):
            for j in range(len(agent[i].movable)):
                if agent[i].movable[j] not in agent_data.GetPosition:
                    self.panel[int(agent[i].movable[j]%100)-1][int(agent[i].movable[j]/100)-1]\
                    .SetBackgroundColour('')
            for j in range(len(agent[i].removable)):
                if agent[i].movable[j] not in agent_data.GetPosition:
                    self.panel[int(agent[i].removable[j]%100)-1][int(agent[i].removable[j]/100)-1]\
    ...
    def NextColoring(self, next, col, clear=False):
        ...
        """
        if clear:
            self.panel[int(next%100)-1][int(next/100)-1]\
            .SetBackgroundColour('')
        else:
            self.panel[int(next%100)-1][int(next/100)-1]\
            .SetBackgroundColour(col)
            self.log.LogWrite('Coloring of next ({},{}):{}\n'\
                            .format(int(next/100), next%100, col), logtype=procon29.SYSTEM_LOG)
